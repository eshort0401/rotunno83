...
\end{equation}
\item
Note my (and Rotunno's) expressions for Fourier transforms are missing a factor of $\frac{1}{\pi}$, which explains the additional missing factor of $\pi$ when converting from $Q_0$ in \citet{qian09} to $\tilde{A}$! Now fixed in code. Rotunno code results now match my Qian code results.
\end{enumerate}
