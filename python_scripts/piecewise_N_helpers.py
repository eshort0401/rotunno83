...
def integrate_piecewise_N(
        x, z, t, s, alpha, L, N, H1, zN, A, heat_right=True):
    ...
    for j in prange(0, zN):
        ...
        for i in range(x.size):
            for l in range(t.size):
                ...
                # # Calc u
                u1_ig, u2_ig = calc_u_lower(x[i], z[j], t[l], k, L, N, H1, A)
    ...
    for j in prange(zN, z.size):
        ...
        for i in range(x.size):
            for l in range(t.size):
                ...
                # Calc u
                u1_ig, u2_ig = calc_u_upper(x[i], z[j], t[l], k, L, N, H1, A)
                u[0][l, j, i] = np.trapz(u1_ig, k)
                u[1][l, j, i] = np.trapz(u2_ig, k)
...
# psi functions
@jit(nopython=True)
def calc_psi_lower(x, z, t, k, L, N, H1, A):
...
# u functions
@jit(nopython=True)
def calc_u_lower(x, z, t, k, L, N, H1, A):
