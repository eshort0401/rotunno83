...
def integrate_continuous_N_convective(
        x, z, t, s, alpha, L, N, H1, H2, zN, zN_scaled, A0, D0):
    ...
    for j in range(0, zN):
        ...
        base_fns = [
            fn*1j*np.sqrt(np.pi)*L*k*np.exp(-L**2*k**2/4)/(2*A0**2)
            for fn in base_fns]
    ...
    for j in range(zN+zN_scaled-1, z.size):
        psi_base_1_lf, psi_base_1_mf, psi_base_1_hf = calc_psi_base_upper(
            z[j], k, N, H1, H2, A0, D0, X_p, Y_p, S, T,
...
def calc_mid_forcing_integrands(
        H1, H2, k, N, L, z, zN, zN_scaled, A0, D0, grain=3):
    ...
    Ia = np.array(
        np.exp(-(Zp-1)**2/D0**2)/(pcfd(-1/2, (1+1j)*f)*alpha)).astype(complex)
    Ib = np.array(
        np.exp(-(Zp-1)**2/D0**2)/(pcfd(-1/2, (1-1j)*f)*alpha)).astype(complex)
...
def calc_psi_base_lower(
        z, k, N, H1, H2, A0, D0, X, Y, S, T, B, mode='pos'):
    m = k/A0
    ...
    term_1 = -1/m*(E3(z, D0, m)-E3(0, D0, m))
    term_1 = term_1*(B1*np.exp(1j*m*z)+B2*np.exp(-1j*m*z))

    term_2 = (
        B1*(E1(H1, D0, m)-E1(z, D0, m)) + B2*(E2(H1, D0, m)-E2(z, D0, m)))
...
def calc_psi_base_middle(
        z, k, N, H1, H2, A0, D0, X, Y, beta, gamma, B, D, S, T,
    ...
    if mode == 'pos':
        E4inf = .5*np.sqrt(np.pi)*D0*np.exp(-1/4*m*N*(D0**2*m*N+4*1j*(H2-1)))
        term_4 = 1/(2*1j*m*N*(T-S))*(E4inf-E4(H2, D0, m, N, H2))
        term_4 = term_4*((np.sin(m*H1)-gamma)*Da_z/Da_1+gamma*Db_z/Db_1)
    else:
        E5inf = .5*np.sqrt(np.pi)*D0*np.exp(-1/4*m*N*(D0**2*m*N-4*1j*(H2-1)))
        term_4 = -1/(2*1j*m*N*(T+S))*(E5inf-E5(H2, D0, m, N, H2))
        term_4 = term_4*((np.sin(m*H1)-gamma)*Da_z/Da_1+gamma*Db_z/Db_1)
...
def calc_psi_base_upper(
        z, k, N, H1, H2, A0, D0, X, Y, S, T, beta, gamma, D, mode='pos'):
    ...
    if mode == 'pos':
        ...
        term_1 = -1/(m*P)*np.exp(-1j*m*N*(z-H2))
    term_1 = term_1*(E3(H1, D0, m)-E3(0, D0, m))

    if mode == 'pos':
        term_2 = D*np.exp(1j*m*N*(z-H2))

        term_3_a = E5(z, D0, m, N, H2)-E5(H2, D0, m, N, H2)
        term_3_b = E4(z, D0, m, N, H2)-E4(H2, D0, m, N, H2)
        ...
        E4inf = .5*np.sqrt(np.pi)*D0*np.exp(-1/4*m*N*(D0**2*m*N+4*1j*(H2-1)))
        term_4 = term_4*1/(2*1j*m*N*(T-S))*(E4inf-E4(z, D0, m, N, H2))
        ...
        term_4 = ((T+S)*np.exp(1j*m*N*(z-H2))+(T-S)*np.exp(-1j*m*N*(z-H2)))
        E5inf = .5*np.sqrt(np.pi)*D0*np.exp(-1/4*m*N*(D0**2*m*N-4*1j*(H2-1)))
        term_4 = -term_4*1/(2*1j*m*N*(T+S))*(E5inf-E5(z, D0, m, N, H2))

    return term_1, term_2, (term_3+term_4)


# @jit(nopython=True)
def calc_u_base_lower(z, k, N, H1, H2, A0, D0, X, Y, S, T, B, mode='pos'):
    m = k/A0
    ...
    term_1a = -1/m*np.sin(m*z)*np.exp(-(z-1)**2/D0**2)
    term_1a = term_1a*(B1*np.exp(1j*m*z)+B2*np.exp(-1j*m*z))

    term_1b = -1/m*(E3(z, D0, m)-E3(0, D0, m))
    term_1b = term_1b*(B1*1j*m*np.exp(1j*m*z)+B2*(-1j*m)*np.exp(-1j*m*z))

    term_1 = term_1a+term_1b

    term_2a = (
        -B1*np.exp(1j*m*z)*np.exp(-(z-1)**2/D0**2)
        - B2*np.exp(-1j*m*z)*np.exp(-(z-1)**2/D0**2))
    term_2a = -1/m*np.sin(m*z)*term_2a

    term_2b = (
        B1*(E1(H1, D0, m)-E1(z, D0, m)) + B2*(E2(H1, D0, m)-E2(z, D0, m)))
    term_2b = -np.cos(m*z)*term_2b
...
def calc_u_base_middle(
        z, k, N, H1, H2, A0, D0, X, Y, beta, gamma, B, dB, D, dD, S, T,
        Da_1, Db_1, Da_2, Db_2, mode='pos'):
    ...
    term_2a = D*((1-beta)*dDa_z/Da_2+beta*dDb_z/Db_2)
    term_2b = dD*((1-beta)*Da_z/Da_2+beta*Db_z/Db_2)
    term_2 = term_2a+term_2b

    # term_2 = D*((1-beta)*Da_z/Da_2+beta*Db_z/Db_2)

    term_3a = B*((np.sin(m*H1)-gamma)*dDa_z/Da_1+gamma*dDb_z/Db_1)
    term_3b = dB*((np.sin(m*H1)-gamma)*Da_z/Da_1+gamma*Db_z/Db_1)
...
def calc_u_base_upper(
    ...
    if mode == 'pos':
        ...
        term_3a_a = E5(z, D0, m, N, H2)-E5(H2, D0, m, N, H2)
        term_3a_b = E4(z, D0, m, N, H2)-E4(H2, D0, m, N, H2)
        term_3a = term_3a_a+(T+S)/(T-S)*term_3a_b
        term_3a = term_3a*np.exp(1j*m*N*(z-H2))/2

        term_3b_a = np.exp(-1j*m*N*(z-H2))*np.exp(-(z-1)**2/D0**2)
        term_3b_b = np.exp(1j*m*N*(z-H2))*np.exp(-(z-1)**2/D0**2)
        term_3b = term_3b_a+(T+S)/(T-S)*term_3b_b
        term_3b = term_3b*np.exp(1j*m*N*(z-H2))/(2*1j*m*N)

        term_3 = term_3a+term_3b

        E4inf = .5*np.sqrt(np.pi)*D0*np.exp(-1/4*m*N*(D0**2*m*N+4*1j*(H2-1)))
        term_4a = ((T+S)*np.exp(1j*m*N*(z-H2))-(T-S)*np.exp(-1j*m*N*(z-H2)))
        term_4a = term_4a*1/(2*(T-S))*(E4inf-E4(z, D0, m, N, H2))

        term_4b = ((T+S)*np.exp(1j*m*N*(z-H2))+(T-S)*np.exp(-1j*m*N*(z-H2)))
        term_4b = term_4b*1/(2*1j*m*N*(T-S))*(
            -np.exp(1j*m*N*(z-H2))*np.exp(-(z-1)**2/D0**2))
        ...
        term_3a_a = -(E5(z, D0, m, N, H2)-E5(H2, D0, m, N, H2))
        term_3a_b = -(E4(z, D0, m, N, H2)-E4(H2, D0, m, N, H2))
        term_3a = (T-S)/(T+S)*term_3a_a+term_3a_b
        term_3a = term_3a*(-1)*np.exp(-1j*m*N*(z-H2))/2

        term_3b_a = -(np.exp(-1j*m*N*(z-H2))*np.exp(-(z-1)**2/D0**2))
        term_3b_b = -(np.exp(1j*m*N*(z-H2))*np.exp(-(z-1)**2/D0**2))
        term_3b = (T-S)/(T+S)*term_3b_a+term_3b_b
        term_3b = term_3b*np.exp(-1j*m*N*(z-H2))/(2*1j*m*N)
        ...
        E5inf = .5*np.sqrt(np.pi)*D0*np.exp(-1/4*m*N*(D0**2*m*N-4*1j*(H2-1)))
        term_4a = ((T+S)*np.exp(1j*m*N*(z-H2))-(T-S)*np.exp(-1j*m*N*(z-H2)))
        term_4a = -term_4a*1/(2*(T+S))*(E5inf-E5(z, D0, m, N, H2))
